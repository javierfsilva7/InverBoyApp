...
public class RepositorioCotizacionesStub implements RepositorioCotizaciones {
    ...
    public Cotizacion getLastCotizadorByUser(String user) throws PersistenceException {
        int id = 0;
        Iterator it = cotisdb.entrySet().iterator();
        while (it.hasNext()) {
            Map.Entry e = (Map.Entry) it.next();
            if (cotisdb.get(e.getKey()).getCliente().getNombre().equals(user)) {
                if(id < cotisdb.get(e.getKey()).getId()) id=cotisdb.get(e.getKey()).getId();
            }
        }
        if (id == 0) {
